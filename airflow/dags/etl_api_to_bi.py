    ...
    for page in _fetch_paged(endpoint, {"updated_since": watermark}):
        ...
        with pg.get_conn() as conn:
            with conn.cursor() as cur:
                if table == "bronze.customers":
                    cur.executemany(
                        """
                        INSERT INTO bronze.customers(customer_id,company_name,country,industry,company_size,signup_date,updated_at,is_churned)
                ...
                elif table == "bronze.payments":
                    cur.executemany(
                        """
                        INSERT INTO bronze.payments(payment_id,customer_id,product,amount,currency,status,refunded_amount,fee,payment_method,country,created_at,updated_at)
                    ...
                    cur.executemany(
                        """
                        INSERT INTO bronze.sessions(session_id,customer_id,source,medium,campaign,device,country,pageviews,session_duration_s,bounced,converted,session_start,updated_at)
